shaderfiles["webgl2.fs_grayscott"] = `
  ...
  void main () {
    ...
    float c0=.05;
		...
		const int a=1;
		const int c=2;
		/*for(int i=-3;i<4;i++){//noprotect
			for(int j=-3;j<4;j++){
				float fi=float(i);
				float fj=float(j);
				float fl=0.027*(4.2-length(vec2(fi,fj)));
				lap += texture(tex, posn+vec2(fi* wh_rcp.x, fj* wh_rcp.y)).rg*fl;
			}
		}*/
    lap += textureOffset(tex, posn, ivec2(-a, 0)).rg * 0.20;
    lap += textureOffset(tex, posn, ivec2(+a, 0)).rg * 0.20;
    lap += textureOffset(tex, posn, ivec2( 0,-a)).rg * 0.20;
    lap += textureOffset(tex, posn, ivec2( 0,+a)).rg * 0.20;
    lap += textureOffset(tex, posn, ivec2(-c,-c)).rg * 0.05;
    lap += textureOffset(tex, posn, ivec2(+c,-c)).rg * 0.05;
    lap += textureOffset(tex, posn, ivec2(-a,+c)).rg * 0.05;
    lap += textureOffset(tex, posn, ivec2(+a,+c)).rg * 0.05;
    float nA = dA * lap.r - val.r * val.g * val.g + feed * (1.0 - val.r);
    float nB = dB * lap.g + val.g * val.g * val.r - (feed + kill) * val.g;
